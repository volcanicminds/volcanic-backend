...
1. **`develop` è stato riallineato a `main`** il 6 settembre 2026 con `git branch -f develop main`, senza conservare lo stato precedente (deciso: era fermo al 2024 e non serviva). Il push forzato su `origin/develop` non è più necessario: `origin/develop` è stato aggiornato l'8 settembre 2026 e dal giorno dopo il lavoro sta su `v5`, che contiene tutto `develop`. Cosa farne ora è nella tabella «Fuori piano» di `EVO_STATO.md`.
...
3. **Litestream: CHIUSO il 18 settembre 2026, nessun secondo adattatore.** Resta l'unico adattatore di replica per i contenitori su file; chi vuole una replica gestita usa libSQL remoto (sqld o Turso), che il framework apre già. Un secondo adattatore senza un caso reale sarebbe codice mai provato contro niente. Se un progetto chiederà contenitori cifrati a pagina, sarà un progetto a sé con il suo requisito, non una variante di questa porta.
